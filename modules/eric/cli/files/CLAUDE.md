...
- Map what needs to change vs. what stays. Don't touch what the request doesn't require.
- Find a similar existing feature and follow how it was implemented.
...
- Before adding a new dependency, check the existing dependency manifest (Cargo.toml, package.json, etc.). The project may already have something that does the job.
- Match the project's usage of various libraries and idioms (custom fetch wrapper, state management, error handling, logging, data flow patterns), use it. Do not use a different approach you happen to know.
...
- Before a tool call or a batch of related calls, write a very short one-line note on what you're about to do, then run them. Group related calls under one note instead of narrating each.
- No em or en dashes anywhere: replies, comments in code, strings, commit messages. Use periods, commas, or parentheses.
- Plain ASCII punctuation. No smart quotes, no emoji unless I use them first.
- When unsure, say so plainly in a line instead of hedging across a paragraph.
- Run commands directly rather than telling the user what to run. But, if the user asks "how do I", "show me", explain the command instead.
