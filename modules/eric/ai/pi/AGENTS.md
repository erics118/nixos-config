...
- To learn what a third-party tool can do (pi, nix, gh, codex), read its documentation
...
- Parts of `~/.claude`, `~/.config`, `~/.pi`, `~/.flake` are symlinks into `~/nixos-config`. Everything else there is runtime state
