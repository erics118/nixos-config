{
  flake.modules =
    let
      # ntfyHost lives on both platform bases so the darwin ntfy cli and the
      # linux publishers share one value
      ntfyHostOption = { lib, ... }: {
    ...
    in
    {
      ...
      nixos.base = { lib, ... }: {
        ...
        options.tailnetDomain = lib.mkOption {
          type = lib.types.str;
          default = "dolphin-sailfin.ts.net";
          description = "Tailscale magicdns domain, used for cross-host service urls";
        };
